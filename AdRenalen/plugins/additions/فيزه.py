...
async def check_ccbin(client, message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>من فضلك أعطني البين\nلاحضاره !</b>"
        )
    try:
        await message.delete()
    except:
        pass
    aux = await message.reply_text("<b>سوف تحقق من ذلك . . .</b>")
    bin = message.text.split(None, 1)[1]
    if len(bin) < 6:
        return await aux.edit("<b>❌ بین ليس موجود ❗...</b>")
    try:
...
🚫 لم يتم العثور على بن يرجى المحاولة مرة أخرى """)
