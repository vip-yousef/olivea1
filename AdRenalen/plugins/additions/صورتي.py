...
async def iddlock(client: Client, message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
        ...
        return await message.reply_text("عذرا عزيزي هذا الامر للمشرفين بس 💎 ،")
...
async def muid(client: Client, message):
    if message.chat.id in stayle_pic:
        return await message.reply_text("صورتي معطل اطلب من المشرفين تفتحه 💎 ،")
    usr = await client.get_users(message.from_user.id)
    name = usr.first_name
    async for photo in client.get_chat_photos(message.from_user.id, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""{choice(The_Stayle_Pic)}  💎 ،""", 
         reply_markup=InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton(
                        "‹ 𖥳 𝙲𝙷𝙰𝙽𝙽𝙴𝙻 𖥳 ›", url=f"https://t.me/SOURCEOLIVEA")
                ]
            ]
        ),
    )
